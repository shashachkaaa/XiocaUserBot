...
async def ml(client, message):
	...
	if os.path.exists(f"{BASE_PATH}/modules/custom_modules/{module_name}.py"):
		...
		await client.send_document(message.chat.id, document=f'{BASE_PATH}/modules/custom_modules/{module_name}.py', caption = f'<emoji id=5372905603695910757>🌙</emoji> Это модуль {module_name}')
	elif os.path.exists(f"{BASE_PATH}/modules/{module_name}.py"):
